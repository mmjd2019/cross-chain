...
def deploy_contract(url, contract_name, private_key, constructor_args=None):
    ...
    if constructor_args:
        ...
        try:
            if contract_name == 'SimpleToken' and len(constructor_args) == 4:
                # 编码参数: name, symbol, decimals, initialSupply
...
def test_contract_with_abi(url, contract_address, contract_name, abi):
    ...
    if contract_name == "SimpleToken":
        ...
        try:
            # 测试totalSupply函数
            supply_call = encode_function_call("totalSupply()")
            response = call_rpc(url, "eth_call", [{
                "to": contract_address,
                "data": "0x" + supply_call
            }, "latest"])
            ...
            if response and 'result' in response:
                result = response['result']
                if result != "0x":
                    print(f"   ✅ totalSupply函数调用成功: {result}")
                else:
                    print(f"   ⚠️  totalSupply函数返回空")
        except Exception as e:
            print(f"   ❌ totalSupply函数调用出错: {e}")
    
    elif contract_name == "SimpleAssetManager":
        try:
...
def main():
    ...
    for chain_id, chain_info in deployed_contracts.items():
        ...
        token_address = deploy_contract(
            ...
            'SimpleToken', 
            test_private_key,
            [token_name, token_symbol, 18, 1000000 * 10**18]
        )
        ...
        if token_address:
            ...
            test_contract_with_abi(chain_info['url'], token_address, 'SimpleToken', token_abi)
        
        # 2. 部署SimpleAssetManager
        ...
        if asset_manager_address:
            ...
            # 加载ABI进行测试
            with open('SimpleAssetManager.json', 'r') as f:
                asset_manager_abi = json.load(f)['abi']
            test_contract_with_abi(chain_info['url'], asset_manager_address, 'SimpleAssetManager', asset_manager_abi)
        ...
        if contracts:
            ...
            print(f"✅ {chain_info['name']} 简化合约部署完成")
        else:
            print(f"❌ {chain_info['name']} 简化合约部署失败")
    
    # 保存部署结果
    if deployment_results:
        with open('simple_contracts_deployment_results.json', 'w', encoding='utf-8') as f:
            json.dump(deployment_results, f, indent=2, ensure_ascii=False)
        
        print(f"\n📄 简化合约部署结果已保存到: simple_contracts_deployment_results.json")
