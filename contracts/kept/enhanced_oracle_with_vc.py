...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('oracle_with_vc.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class EnhancedOracleWithVC:
    """增强版跨链Oracle服务 - 集成VC功能"""
    ...
    async def monitor_chain_events(self, chain_id: str):
        ...
        last_block = chain_info['web3'].eth.block_number
    ...
    async def process_new_blocks(self, chain_id: str, from_block: int, to_block: int):
        ...
        try:
            # 这里应该检查AssetLocked事件
            # 由于我们没有部署完整的合约，我们模拟事件处理
            logger.info(f"处理链 {chain_id} 区块 {from_block}-{to_block}")
            
            # 模拟检测到锁定事件
            if to_block % 10 == 0:  # 每10个区块模拟一个事件
                    ...
                    if response.status == 200:
                        ...
                        async with session.post(
                            ...
                            if holder_response.status == 200:
                                ...
                                logger.error("持有者接收邀请失败")
                                return None
                    else:
                        logger.error("发行者创建邀请失败")
                        return None
