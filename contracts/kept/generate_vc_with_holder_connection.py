...
def generate_vc_with_holder_connection():
    ...
    if response.status_code == 200:
        ...
        if active_connections:
            ...
            print(f"✅ 找到持有者端活跃连接: {connection_id}")
            print(f"   对方DID: {their_did}")
        else:
            print("❌ 持有者端没有活跃连接")
            return False
    else:
        print(f"❌ 获取持有者端连接失败: HTTP {response.status_code}")
        return False
    ...
    if response.status_code == 200:
        ...
        if issuer_connection:
            ...
            print(f"✅ 找到发行者端对应连接: {issuer_connection_id}")
        else:
            print("❌ 发行者端没有对应的连接")
            return False
    else:
                ...
                if response.status_code == 200:
                    ...
                    if vc_record:
                        ...
                        if state == 'credential_acked':
                            ...
                            print(f"📁 结果已保存到: holder_connection_vc_result.json")
                            return True
                        else:
