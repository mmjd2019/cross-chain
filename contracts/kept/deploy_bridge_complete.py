...
def main():
    ...
    for chain_id, chain_info in deployed_contracts.items():
        ...
        # 2. 尝试部署CrossChainBridge（无参数版本）
        # 首先创建一个无参数的CrossChainBridge
        print(f"\n🔧 尝试部署CrossChainBridge...")
        cross_chain_bridge_address = deploy_contract(chain_info['url'], 'CrossChainBridge', test_private_key)
        if cross_chain_bridge_address:
            contracts['cross_chain_bridge'] = cross_chain_bridge_address
            test_contract(chain_info['url'], cross_chain_bridge_address, 'CrossChainBridge')
    ...
    if deployment_results:
        ...
        print(f"\n📄 桥合约部署结果已保存到: bridge_deployment_results.json")
